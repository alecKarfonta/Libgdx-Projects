...
public class Settings implements Screen {
		...
		ClickListener buttonHandler = new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				...
				if(event.getListenerActor() == vSyncCheckBox) {
					// save vSync
					Gdx.app.getPreferences("title").putBoolean("vsync", vSyncCheckBox.isChecked());

					// set vSync
					Gdx.graphics.setVSync(vSync());

					Gdx.app.log("title", "vSync " + (vSync() ? "enabled" : "disabled"));
				} else if(event.getListenerActor() == back) {
					// save level directory
					String actualLevelDirectory = levelDirectoryInput.getText().trim().equals("") ? Gdx.files.getExternalStoragePath() + "title" + "/levels" : levelDirectoryInput.getText().trim(); // shortened form of an if-statement: [boolean] ? [if true] : [else] // String#trim() removes spaces on both sides of the string
					Gdx.app.getPreferences("title").putString("leveldirectory", actualLevelDirectory);
					...
					Gdx.app.log("title", "settings saved: " + levelDirectoryInput.getText() );
	...
	public static FileHandle levelDirectory() {
		String prefsDir = Gdx.app.getPreferences("title").getString("leveldirectory").trim();
		// if the default level directory (.TITLE/levelDirectory) exists return it or 
		if(prefsDir != null && !prefsDir.equals(""))
			return Gdx.files.absolute(prefsDir);
		else
			return null; // Gdx.files.absolute(Gdx.files.external("title" + "/levels").file().getAbsolutePath()); // return default level directory
	}

	/** @return if vSync is enabled */
	public static boolean vSync() {
		return Gdx.app.getPreferences("title").getBoolean("vsync");
	}
